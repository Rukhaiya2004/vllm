 ...
 public:
  static void dequant(int32_t* __restrict__ q_weight,
    ...
    for (int32_t k_idx = 0; k_idx < k_size; k_idx += 8) {
      ...
      if constexpr (isa == ISA::VSX && std::is_same_v<scalar_t, c10::BFloat16>) {
        vec_op::dequant_interleave_save_fp32_to_bf16(wb_0, wb_1, curr_weight);
        vec_op::dequant_interleave_save_fp32_to_bf16(wb_2, wb_3, curr_weight + 32);
        vec_op::dequant_interleave_save_fp32_to_bf16(wb_4, wb_5, curr_weight + 64);
        vec_op::dequant_interleave_save_fp32_to_bf16(wb_6, wb_7, curr_weight + 96);
      } else {
...
void cpu_gemm_wna16_impl(
  ...
  // a schedule policy designed to fit in L2/L3 cache blocks and maximize per-thread work
  const int32_t n_partition_size = [&]() {
    ...
    ps_thread_limit =
        std::max(((ps_thread_limit + gemm_n_tile_size - 1) / gemm_n_tile_size) * gemm_n_tile_size,
                 (int64_t)gemm_n_tile_size);
  ...
  // get buffer size
  // Allocate 2 ping-pong tile buffers for B to double-buffer dequantization and GEMM
...
#pragma omp parallel for schedule(static, 1)
  for (int32_t thread_id = 0; thread_id < thread_num; ++thread_id) {
    ...
    for (;;) {
      ...
      scalar_t* __restrict__ init_bias = (bias != nullptr) ? (bias + n_start_idx) : nullptr;
      scalar_t* __restrict__ init_output = output + n_start_idx;

      // Helper lambda to dequantize a specific tile into the designated ping-pong buffer
      auto dequant_tile = [&](int32_t tile_idx, scalar_t* dest_b_buf) {
        const int32_t tile_n_block_start = n_block_start_idx + tile_idx * blocks_per_tile;
        const int32_t tile_n_start = n_start_idx + tile_idx * gemm_n_tile_size;
        int32_t* __restrict__ curr_q_weight = q_weight + tile_n_block_start * q_weight_block_stride;
      ...
      if (n_tile_num > 0) {
        ...
        for (int32_t n_tile_idx = 0; n_tile_idx < n_tile_num; ++n_tile_idx) {
          scalar_t* curr_tile_b_buffer = (n_tile_idx % 2 == 0) ? b_buf0 : b_buf1;
          scalar_t* next_tile_b_buffer = (n_tile_idx % 2 == 0) ? b_buf1 : b_buf0;

          // Pipeline: Dequantize next tile (if any) while current tile is ready for GEMM
          ...
          scalar_t* __restrict__ curr_output = init_output + n_tile_idx * gemm_n_tile_size;
          scalar_t* __restrict__ curr_bias = (init_bias != nullptr) ? (init_bias + n_tile_idx * gemm_n_tile_size) : nullptr;

          for (int32_t m_idx = 0; m_idx < m_size; m_idx += gemm_m_tile_size) {
            const int32_t curr_m_size = std::min(gemm_m_tile_size, m_size - m_idx);

            gemm.gemm(curr_input, curr_tile_b_buffer, c_buffer, curr_m_size, k_size,
