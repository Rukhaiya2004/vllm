...
class TileGemmVSX {
 public:
  ...
  static void gemm_micro_vsx_fallback(DEFINE_CPU_MICRO_GEMM_PARAMS) {
    ...
    const __vector unsigned char perm_k0 = {0, 1, 16, 17, 4,  5,  16, 17,
                                            8, 9, 16, 17, 12, 13, 16, 17};
    const __vector unsigned char perm_k1 = {2,  3,  16, 17, 6,  7,  16, 17,
                                            10, 11, 16, 17, 14, 15, 16, 17};
    ...
    for (int32_t k_idx = 0; k_idx < k; k_idx += 2) {
      ...
      // In our layout, b_ptr[k_idx * 16 + j * 8] contains 4 pairs of [k0, k1]
      // for 4 columns
      __vector unsigned short vB0 = (__vector unsigned short)vec_xl(
          0, (const unsigned char*)&b_ptr[k_idx * 16 + 0]);
      __vector unsigned short vB1 = (__vector unsigned short)vec_xl(
          0, (const unsigned char*)&b_ptr[k_idx * 16 + 8]);
      __vector unsigned short vB2 = (__vector unsigned short)vec_xl(
          0, (const unsigned char*)&b_ptr[k_idx * 16 + 16]);
      __vector unsigned short vB3 = (__vector unsigned short)vec_xl(
          0, (const unsigned char*)&b_ptr[k_idx * 16 + 24]);
...
#pragma GCC unroll 2
      for (int i = 0; i < M; i++) {
        __vector unsigned short vA = (__vector unsigned short)vec_splats(
            *reinterpret_cast<const uint32_t*>(&a_ptr[i * lda + k_idx]));
